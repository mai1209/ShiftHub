...
export default function PaymentSettingsScreen() {
  ...
  const advanceHelper = useMemo(() => {
    if (!form.advancePaymentEnabled)
      return 'El cliente solo ve pago en el local.';
    if (form.mercadoPagoConnectionStatus !== 'connected')
      return 'Para mostrar el pago online, conectá Mercado Pago.';
    if (form.advanceMode === 'full')
      return 'El cliente paga el turno completo antes de confirmar.';
  ...
  const handleSave = async () => {
    try {
      ...
      if (!Number.isFinite(parsedAdvanceValue) || parsedAdvanceValue < 0) {
        Alert.alert(
          'Valor inválido',
          'Revisá el monto o porcentaje del adelanto.',
        );
        return;
      }
  ...
  const handleConnectMercadoPago = async () => {
    try {
      ...
      setForm(current => ({
        ...current,
        mercadoPagoConnectionStatus: 'pending',
      }));
    } catch (err: any) {
      Alert.alert(
        'No pudimos abrir Mercado Pago',
        err?.message ??
          'Revisá la configuración del backend y volvé a intentar.',
      );
    } finally {
      setConnecting(false);
    }
  ...
  const handleDisconnectMercadoPago = async () => {
    try {
      ...
      Alert.alert(
        'No pudimos desconectar la cuenta',
        err?.message ?? 'Probá de nuevo.',
      );
    } finally {
      setConnecting(false);
    }
        ...
        {form.advancePaymentEnabled && (
          <View style={styles.nestedBlock}>
            ...
            <View style={styles.segmentRow}>
              <SegmentButton
                label="Seña"
                active={form.advanceMode === 'deposit'}
                onPress={() => setForm(c => ({ ...c, advanceMode: 'deposit' }))}
                theme={theme}
                styles={styles}
              />
              <SegmentButton
                label="Total"
                active={form.advanceMode === 'full'}
                onPress={() => setForm(c => ({ ...c, advanceMode: 'full' }))}
                theme={theme}
                styles={styles}
              />
            </View>
            ...
            {form.advanceMode === 'deposit' && (
              <>
                ...
                <View style={styles.segmentRow}>
                  <SegmentButton
                    label="Porcentaje"
                    active={form.advanceType === 'percent'}
                    onPress={() =>
                      setForm(c => ({ ...c, advanceType: 'percent' }))
                    }
                    theme={theme}
                    styles={styles}
                  />
                  <SegmentButton
                    label="Fijo"
                    active={form.advanceType === 'fixed'}
                    onPress={() =>
                      setForm(c => ({ ...c, advanceType: 'fixed' }))
                    }
                    theme={theme}
                    styles={styles}
                  />
...
function SegmentButton({
  ...
  return (
    ...
    >
      <Text
        style={[styles.segmentButtonText, active && { color: theme.primary }]}
      >
        {label}
      </Text>
    </Pressable>
...
const createStyles = (theme: any) =>
  StyleSheet.create({
    ...
    statusNoticeConnected: {
      backgroundColor: 'rgba(49, 201, 108, 0.12)',
      borderColor: 'rgba(49, 201, 108, 0.35)',
    },
    statusNoticePending: {
    ...
    statusNoticeTitleConnected: {
      color: theme.mode === 'light' ? '#15803D' : '#66DA92',
    },
    statusNoticeTitlePending: {
      color: theme.mode === 'light' ? '#B7791F' : '#F6C453',
    },
    statusNoticeTitleDisconnected: { color: theme.textPrimary },
    statusNoticeText: { fontSize: 12, lineHeight: 18 },
    statusNoticeTextConnected: {
      color: theme.mode === 'light' ? '#166534' : '#DFF8E8',
    },
    statusNoticeTextPending: {
      color: theme.mode === 'light' ? '#92400E' : '#F6E1A9',
    },
