...
export type BusinessCopy = {
  ...
  staffPlural: string;
  staffSingularCapitalized: string;
  staffPluralCapitalized: string;
  teamTitle: string;
};

const DEFAULT_BUSINESS_TYPE: BusinessType = 'other';
...
export function normalizeBusinessType(value: unknown): BusinessType {
  const normalized = String(value ?? '').trim().toLowerCase() as BusinessType;
  return BUSINESS_TYPE_OPTIONS.some(option => option.value === normalized)
    ? normalized
    : DEFAULT_BUSINESS_TYPE;
}

export function getBusinessCopy(value?: unknown): BusinessCopy {
  ...
  const overrides: Partial<Record<BusinessType, Partial<typeof base>>> = {
    beauty_center: {
